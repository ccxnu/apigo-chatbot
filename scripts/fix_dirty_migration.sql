...
-- Run this manually with: psql -h localhost -U postgres -d chatbot_db -f scripts/fix_dirty_migration.sql

-- First, drop the function if it exists (in case it was partially created)
DROP FUNCTION IF EXISTS fn_get_conversation_history(VARCHAR, INT);

-- Recreate it with the correct signature
...
COMMENT ON FUNCTION fn_get_conversation_history IS 'Get message history for a conversation including sender type';

-- Fix the schema_migrations table to mark migration 27 as complete
UPDATE schema_migrations SET dirty = false WHERE version = 27;
