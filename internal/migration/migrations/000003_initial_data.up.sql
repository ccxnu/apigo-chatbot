...
begin
    ...
    -- Analytics cost configuration
    if not exists (select 1 from cht_parameters where prm_code = 'COST_CONFIG') then
        insert into cht_parameters (prm_name, prm_code, prm_data, prm_description)
        values ('ANALYTICS', 'COST_CONFIG', '{"llmPricePerMillionInputTokens": 0.50, "llmPricePerMillionOutputTokens": 1.50, "embeddingPricePerMillionTokens": 0.13}'::jsonb, 'LLM and embedding pricing for cost calculation in analytics');
    end if;
